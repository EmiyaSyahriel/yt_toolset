...
pub fn Color.rgba_from_u32(src u32, param ColorParseParam) Color {
	return Color.rgba(rs2u8(src, 24), rs2u8(src, 16), rs2u8(src, 8), rs2u8(src, 0), param)
}

pub fn Color.bgra_from_u32(src u32, param ColorParseParam) Color {
	return Color.bgra(rs2u8(src, 24), rs2u8(src, 16), rs2u8(src, 8), rs2u8(src, 0), param)
}
...
pub fn Color.parse(str string) !Color {
	...
	if k_str.len == 8 {
		return Color.bgra(Color.parse_single_hex(k_str[0..2])!,
			Color.parse_single_hex(k_str[2..4])!, Color.parse_single_hex(k_str[4..6])!,
			Color.parse_single_hex(k_str[6..8])!)
	} else if k_str.len == 6 {
		return Color.bgr(Color.parse_single_hex(k_str[0..2])!,
			Color.parse_single_hex(k_str[2..4])!, Color.parse_single_hex(k_str[4..6])!)
	} else {
